...
new_braker.gtf -> braker_eggnog_old.gtf
```
...
cat braker_eggnog_old.gtf | grep -w "gene" | wc -l
> 11902
```
...
unannotated_scaffolds_NEW.txt -> assembly/SAMBA
```
...
```
sed 's/\t/ | /g; 1s/^/| /; 1s/$/ |/' scaffolds_stats_NEW | \
sed '2s/.*/|---|---|---|---|---|/' > table_scaffold_stats.md
```

```
awk 'NR==1{print; next} {sub(/c1-.*/, "", $1); print}' table_scaffold_stats.md > table_scaffold_stats_name.md
```
